...
@app.route("/resultados_locales", methods=['GET', 'POST'])

def scrapear_locales():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            while siguiente:
                time.sleep(random.randint(3,7))
                base_url = f"https://www.idealista.com/{regimen.lower()}-locales/{ciudad.lower()}-{provincia.lower()}/con-locales/pagina-{pagina}.htm?ordenado-por=precios-asc"
                print(base_url)
                web = requests.get(base_url, headers=headers)
                soup = bs(web.content, 'html.parser')
...
@app.route("/download_locales", methods=['GET','POST'])

def get_table_locales(): # ESTA ES LA BUENA
